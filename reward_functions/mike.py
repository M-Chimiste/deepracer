...
# waypoints generated based on a menger curve of the track
...
# thanks to https://github.com/TwoDigits/deepracer/blob/master/reward_function.py
...
def new_get_waypoints_ordered_in_driving_direction(params):
    # waypoints are always provided in counter clock wise order
    if params['is_reversed']: # driving clock wise.
...
def get_target_point(params):
    waypoints = up_sample(new_get_waypoints_ordered_in_driving_direction(params), 10)
    backup_waypoints = up_sample(get_waypoints_ordered_in_driving_direction(params), 20)
    ...
    if params['all_wheels_on_track']:  
        ...
        distances = [dist(p, car) for p in backup_waypoints]
        min_dist = min(distances)
        i_closest = distances.index(min_dist)

        n = len(backup_waypoints)

        backup_waypoints_starting_with_closest = [backup_waypoints[(i+i_closest) % n] for i in range(n)]

        r = params['track_width'] * 0.9

        is_inside = [dist(p, car) < r for p in backup_waypoints_starting_with_closest]
        i_first_outside = is_inside.index(False)

        if i_first_outside < 0:  # this can only happen if we choose r as big as the entire track
            return backup_waypoints[i_closest]

        return backup_waypoints_starting_with_closest[i_first_outside]
